...
:: optimized for speed, 32 bit arch. 
bjam.exe -a -j%COMPILATION_NUMCORES% --toolset=msvc --layout=system cxxflags="-D_WIN32_WINNT=0x0600" cflags="-D_WIN32_WINNT=0x0600" optimization=speed link=shared threading=multi address-model=32 --stagedir="stage\msvc\Release x86" release stage
...
:: iostreams, 32 bit arch. 
bjam.exe -a -j%COMPILATION_NUMCORES% --toolset=msvc --layout=system cxxflags="-D_WIN32_WINNT=0x0600" cflags="-D_WIN32_WINNT=0x0600" optimization=speed link=shared threading=multi address-model=32 --stagedir="stage\msvc\Release x86" --with-iostreams -sNO_COMPRESSION=0 -sNO_ZLIB=0 -sBZIP2_SOURCE=%BZIP_SRC% -sZLIB_SOURCE=%ZLIB_SRC% release stage
...
:: debug libs, 32 bit arch. 
bjam.exe -a -j%COMPILATION_NUMCORES% --toolset=msvc --layout=system cxxflags="-D_WIN32_WINNT=0x0600" cflags="-D_WIN32_WINNT=0x0600" link=shared threading=multi address-model=32 --stagedir="stage\msvc\Debug x86" debug stage

:: Fourth pass, specifically build multithreaded debug dlls for 
:: iostreams, 32 bit arch. 
bjam.exe -a -j%COMPILATION_NUMCORES% --toolset=msvc --layout=system cxxflags="-D_WIN32_WINNT=0x0600" cflags="-D_WIN32_WINNT=0x0600" link=shared threading=multi address-model=32 --stagedir="stage\msvc\Debug x86" --with-iostreams -sNO_COMPRESSION=0 -sNO_ZLIB=0 -sBZIP2_SOURCE=%BZIP_SRC% -sZLIB_SOURCE=%ZLIB_SRC% debug stage

:: Recursively search for PDB files in debug directory, copy them to 
:: the debug stage directory.
for /R "%cd%\bin.v2\libs\" %%f in (*.pdb) do xcopy /Y %%f "%cd%\stage\msvc\Debug x86\lib\"
